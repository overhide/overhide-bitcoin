...
/**
 * Job function to update most recent btcereum blocks.
 */
async function go() {
  ...
  try {
    ...
    if (dbMaxBlock === -1) {
      const transactions = await btc.getTransactionsForBlock(latestBlock);
    ...
    for(var block = dbMaxBlock + 1; block <= latestBlock; block++) {
      const transactions = await btc.getTransactionsForBlock(block);
